...
const Sidebar = ({ isOpen, toggleSidebar }) => {
    const twoSegments = window.location.pathname;

    // Sirf pehle do segments extract karna
    const currentURL = twoSegments.split('/').slice(0, 3).join('/');  // 0 se start karke 2 tak lena (0-based index)
    const currentURL2 = twoSegments.split('/').slice(0, 2).join('/');  // 0 se start karke 1 tak lena (0-based index)
    ...
    // console.log('liHover', liHover)
    const [sideW, setSideW] = useState(false)
    const combinedClickHandler = (index) => {
        setSideW(!sideW)
        setIsSubmenu(!showAddShorts)
        setActiveItem(index)
    ...
    const menuItems = [
        ...
        },
        {
            ...
            submenu: [
                ...
                { label: 'All Job List', path: '/all-job-list', detail: '/job-details', add: '/add-job' },
                { label: 'All Applicant List', path: '/applicant_list', detail: '/applicant_details', add: '/add_applicant' },
                { label: 'All Employee List', path: '/all-employee-list', detail: '/employee-details', add: '/add-employee' },
        ...
        },
        {
            ...
            submenu: [
                { label: 'Department', path: '/department', detail: '/department_details', },
                { label: 'Designation', path: '/designation', detail: '/designationdeatils', },
                { label: 'Employee Health', path: '/health', detail: '/employeehealthdetails', add: '/addemployeehealth' },
                { label: 'Birthday', path: '/birthday' },
                { label: 'Announcements', path: '/announcements', detail: '/announcements-details', add: '/add-AddAnnouncements' },
            ]
        },
        {
            label: 'Leave Tracker',
            ...
            submenu: [
                ...
                { label: 'All Leave', path: '/all-leave', detail: '/leave-details', add: '/new-leave' },
                { label: 'Leave Type', path: '/leave_type', detail: '/leave_type_details', },
                { label: 'Travel', path: '/travel', detail: '/travel-details', add: '/add-travel' },
        ...
        },
        {
            ...
            submenu: [
                ...
                { label: 'Holiday', path: '/holiday', detail: '/holiday-details', },
                { label: 'Shift Master', path: '/shift', detail: '/shift-details', },
                { label: 'Assign Shift ', path: '/new-shift',  },
                { label: 'All Attendance List', path: '/all-attendance-list', detail: '/attendance-details', },
            ]
        },
        {
            label: 'Work',
            ...
            submenu: [
                { label: 'Ticket', path: '/ticket', detail: '/ticket-details', add: '/add-ticket' },
                { label: 'Trainers', path: '/trainers', detail: '/trainer-details', add: '/add-trainer' },
                { label: 'Training', path: '/training', detail: '/training-details', add: '/new-training' },
                { label: 'Project ', path: '/project', detail: '/project-details', add: '/add-project' },
                { label: 'Client', path: '/client', detail: '/client-details', add: '/new-client' },
            ]
        }
    ];

    // useEffect(() => {
    //     if (currentURL === '/file/employee') {
    //         setActiveItem(null);
    //     }
    //     const pathIndex = menuItems.findIndex(item =>
    //         item.submenu.some(sub => sub.path === currentURL) || item.submenu.some(sub => sub.detail == currentURL2)
    //     );
    //     setActiveItem(pathIndex)
    // }, [currentURL, liHover]);

    useEffect(() => {
        let pathIndex = menuItems.findIndex(item =>
            item.submenu.some(sub => sub.path === currentURL)
        );

        // Agar currentURL match nahi karta toh currentURL2 ko check karein
        if (pathIndex === -1) {
            pathIndex = menuItems.findIndex(item =>
                item.submenu.some(sub => sub.detail === currentURL2) || item.submenu.some(sub => sub.add === currentURL2)
            );
        }

        // Active item ko set karein
        setActiveItem(pathIndex);
    }, [currentURL, currentURL2, liHover]);

    const handleMouseEnter = () => {
        dispatch(setLiHover(true));
    };

    const handleMouseLeave = () => {
        dispatch(setLiHover(false));
    };
                                ...
                                <div className={`hover_menu ${activeItem === index ? 'show' : ''} ${activeItem2 || !liHover ? 'active_span_li' : 'active_span_li_2'}`}>
                                    ...
                                    <ul>
                                        {item.submenu.map((submenuItem, subIndex) => (
                                            <li
                                                className={`${currentURL == submenuItem.path ? 'li_open' : ''
                                                    || currentURL2 == submenuItem.detail ? 'li_open' : ''
                                                        || currentURL2 == submenuItem.add ? 'li_open' : ''}`}
                                                key={subIndex}
                                                onClick={() => {
                    ...
                    <div className={` ${isOpen ? '' : 'side'} `}>
                        <div className={` ${isOpen ? '' : 'sideLine'} `}>
                            ...
                            <div className="ul22">
                                ...
                                // style={{ height: `150px` }}
                                >
                                    <li className={`${currentURL === '/file/employee' || currentURL === '/file/organization' ? 'li bli' : ''}`}>
                                    ...
                                    <li className={`${currentURL === '/performance' ? 'li bli' : ''}`}>
                                        <span className='spanLiP'>
                                            <span onClick={() => navigate('/performance')}><TbGraph /></span>{isOpen && <p className={` ${isOpen ? 'openP' : 'closeP'}`}>Performance</p>}  {isOpen ? '' : <div className='hover_P'><p>Performance</p> <div></div></div>}
                                        </span>
                                    </li>
                                    {/* <li className={`${currentURL === '/all-employee-list' ? 'li bli' : ''}`}>
                                        <span className='spanLiP'>
                                            <span onClick={() => navigate('/all-employee-list')}><TbFolderSymlink /></span>{isOpen && <p className={` ${isOpen ? 'openP' : 'closeP'}`}>Onboarding</p>}  {isOpen ? '' : <div className='hover_P'><p>Onboarding</p> <div></div></div>}
                                        </span>
                                    </li> */}
                                    <li className={`${currentURL === '/reports/my' || currentURL === '/reports/organizational' ? 'li bli' : ''}`} style={{ marginBottom: "20px" }}>
                                        <span className='spanLiP'>
