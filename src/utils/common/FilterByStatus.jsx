...
const FilterByStatus = ({
  ...
  const filterButtonClick = OutsideClick();
  const handleFilter = (name) => {
    if (name === selectedSortBy) {
      return;
    }
    if (name === "all" && selectedSortBy === "all") {
      return; // Do nothing if "All" is already selected
    }
    ...
    setTimeout(() => {
      filterButtonClick?.handleToggle();
    }, 100);
  };
  return (
    <>
      <div id='filter_left' onClick={filterButtonClick?.handleToggle}
        ref={filterButtonClick?.buttonRef}>
      ...
      <div
        className={`left ${!filterButtonClick.isOpen ? "filterLeftOpen" : `filterLeftClose ${topCss}`}`}
        ref={filterButtonClick.ref}
      >
        <div onClick={() => handleFilter("")}>
          <div
            className={`active ${selectedSortBy === "all" ? "listActiveStatus" : ""
              }`}
          >
            <span>
              <FaList />
        ...
        {filterOptions?.map((option) => (
          <div className="all">
            <div
              className={selectedSortBy == option.id ? "listActiveStatus" : ""}
