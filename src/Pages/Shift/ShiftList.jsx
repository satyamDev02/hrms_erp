...
import { filterLeavesTypeOptions } from "../../utils/FilterConstant";
...
const NewShift = () => {
  ...
  const handleUpdateStatus = () => {
    ...
    dispatch(updateNewShiftMasterStatus(sendData))
      .then((res) => {
        if (res.success) {
  ...
  const handleSelectAll = () => {
    ...
    setSelectedRows(selectAll ? [] : shiftLists?.map((row) => row.id));
  };
  //checkbox click
  ...
  useEffect(() => {
    if (searchTrigger) fetchShiftList();
  }, [searchTrigger]);
  ...
  const handleRowClicked = (item) => {
    navigate(`/shift-details/${item?.id}`);
  };
  ...
  return (
    <div id="allEmp">
      ...
      {newShiftPopup.isOpen && (
        <AddNewShiftForm
          ClosePop={newShiftPopup.handleToggle}
        ...
        <div className="EmpOn_header">
          <div className="top-bar">
            <h2>
              ...
              All Shifts list <p>{totalItems} total</p>
            </h2>
            <div className="Emp_Head_Right">
              {/* <div className="addEmp" onClick={() => navigate("/new-shift")}>
                <p>
                  <span></span> Assign shift List
                </p>
              </div> */}
              ...
              <div
                ...
                onClick={newShiftPopup.handleToggle}
              >
                <p>
                  <span>
                    <IoMdAdd />
      ...
      <div className="EmpOn_Second_Head">
        ...
        <div className="right">
          ...
          <SearchBox onSearch={onSearch} placeholder="Search Shift Name, Start Time, End Time..." />
                ...
                {shiftLists.length > 0 ? (
                  shiftLists?.map((item, index) => (
                    <tr key={index}>
                      ...
                      {/* <td onClick={() => handleRowClicked(item)}></td> */}
                      <td onClick={() => handleRowClicked(item)}>
                        {item?.break_time   || "-"}
                      </td>
                      <td>
