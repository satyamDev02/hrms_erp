...
import { getTrainingList, updateTrainingStatus } from "../../Redux/Actions/trainingActions";
...
const Training = () => {
    ...
    const totalItems = trainingData?.data?.count || 0;
    const trainingListLoading = trainingData?.loading || false;
console.log("trainingLists", trainingLists)
    ...
    const handleUpdateStatus = () => {
        ...
        dispatch(updateTrainingStatus(sendData))
            .then((res) => {
                if (res.success) {
    ...
    const handleSelectAll = () => {
        ...
        setSelectedRows(selectAll ? [] : trainingLists?.map((row) => row.id));
    };
    ...
    const fetchProjectList = useCallback(async () => {
        try {
            ...
            dispatch(getTrainingList(sendData));
        } catch (error) {
    ...
    const handleRowClicked = (trainings) => {
        // navigate(`/training-details`);
        navigate(`/training-details/${trainings?.id}`);
    };
   console.log("tanninglist", trainingLists)
    return (
        <div id="allEmp">
            <ConfirmPopup
                open={showModal}
                ...
                <div className="EmpOn_header">
                    <div className="top-bar">
                        <h2>
                            ...
                            All Training list <p>{totalItems} total</p>
                        </h2>
            ...
            <div className="EmpOn_Second_Head">
                ...
                <div className="right">
                    ...
                    <SearchBox
                        onSearch={onSearch}
                        placeholder='Search Traning Type Trainer Name, Employee Name...'
                    />
            ...
            <div className="allEmployeeList">
                <div className="employee-table">
                    {trainingListLoading ? <TableViewSkeleton /> :
                        <table>
                            <thead>
                                <tr>
                                    <th>
                            ...
                            <tbody>
                                {trainingListLoading ? (
                                    ...
                                    <>
                                        {trainingLists?.length >= 1 ? (trainingLists?.map((item, index) => (
                                            <tr key={index}>
                                                <td>
                                                    <input
                                                        type="checkbox"
                                                ...
                                                <td className="td" onClick={() => handleRowClicked(item)}>
                                                    {item?.training_cost ? "$" + item?.training_cost : "-"}
                                                </td>
                                                <td className="td" onClick={() => handleRowClicked(item)}>
                                                    {item?.duration || "-"}
                                                </td>
